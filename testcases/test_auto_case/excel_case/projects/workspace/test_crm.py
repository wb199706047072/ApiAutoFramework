...
# 用例数据
cases = [{'id': 'case_login_excel_01', 'title': '用户名密码正确，登录成功', 'severity': 'NORMAL', 'url': '/api/crm/v4/user/login', 'run': True, 'method': 'POST', 'headers': "{'Content-Type': 'application/json; charset=utf-8;'}", 'cookies': None, 'request_type': 'JSON', 'payload': {'username': 'admin', 'password': "'123123'", 'appPlatform': 'work-space', 'appVersion': '1.0.1'}, 'files': None, 'wait_seconds': None, 'validate': "{'eq': {'http_code': 200, '$.username': admin}", 'extract': "{'username': '$.data.user.username', 'openid': '$.data.user.openid'}", 'case_dependence': None}, {'id': 'case_login_excel_02', 'title': '用户名正确，密码错误，登录失败', 'severity': 'TRIVIAL', 'url': '/api/crm/v4/user/login', 'run': False, 'method': 'POST', 'headers': "{'Content-Type': 'application/json; charset=utf-8;'}", 'cookies': None, 'request_type': 'JSON', 'payload': {'username': 'admin', 'password': "'12313'", 'appPlatform': 'work-space', 'appVersion': '1.0.1'}, 'files': None, 'wait_seconds': None, 'validate': "{'eq': {'http_code': 200, '$.status': -2}}", 'extract': '', 'case_dependence': None}, {'id': 'case_login_excel_03', 'title': '用户名错误，密码正确，登录失败', 'severity': 'TRIVIAL', 'url': '/api/crm/v4/user/login', 'run': False, 'method': 'POST', 'headers': "{'Content-Type': 'application/json; charset=utf-8;'}", 'cookies': None, 'request_type': 'JSON', 'payload': {'username': 'admins', 'password': "'12313'", 'appPlatform': 'work-space', 'appVersion': '1.0.1'}, 'files': None, 'wait_seconds': None, 'validate': "{'eq': {'http_code': 200, '$.status': -2}}", 'extract': None, 'case_dependence': None}]
@allure.epic("Default Epic")
@allure.feature("test_crm")
@allure.story("Default Story")
@pytest.mark.auto
@pytest.mark.excel_case
@pytest.mark.projects
@pytest.mark.workspace
@pytest.mark.parametrize("case", cases, ids=lambda x: x["title"])
def test_crm_auto(case):
    # 前置依赖处理
